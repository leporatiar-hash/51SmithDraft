 ...
 * teams counts once for each side. `owners`/`ownerOf` come from whichever
 * roster is currently in effect — the static config, or a completed draft.
 */
export function standings(games, owners, ownerOf) {
  const table = new Map(
    owners.map((o) => [
      o.name,
